...
bool cache::checkadrL1(string adr){
	int adr_index = bi2de(adr.substr(0,3));
	int adr_tag = bi2de(adr.substr(4,11));
...
bool cache::controller(bool MemR, bool MemW, int* data, int adr, int* myMem)
{
	...
	int adr_index = bi2de(real_adr.substr(0, 3));
	int adr_tag = bi2de(real_adr.substr(4, 11));
